...
my_file = os.path.join(directory, reference_image + '.txt')
...
scriptfile = os.path.join(directory, 'SEM_to_autocad_ref_'+reference_image+'.scr')
try:
...
def save_image_positions(filepath, filename):
    ...
    if config.has_section('SemImageFile'):
        ...
        if (image_name != reference_image+'.tif'):
            append_to_autocad_script(os.path.join(directory, image_name[:-4] + '.tif'), orig_x, orig_y, orig_scale, orig_rotation)
